...
export function portraitAvailability(code, record, portraitDocument) {
  ...
  if (
    ...
    || portraitDocument.permission !== "required-for-production"
...
export function buildFactModel(record, { compact = true } = {}) {
  ...
  sections.push({ label: "Source", items: ["FIFA"] });
...
function socialCreditLink({ href, label, text, pathData }) {
  ...
  link.append(icon, handle);
  return link;
}

function renderModel(host, model, prefix, headingTag = "h4") {
...
export function initMatchDetails(root, detailsState, portraitState) {
  ...
  function renderDisclosure() {
    ...
    const link = document.createElement("a");
    link.href = PORTRAIT_HOST;
    link.target = "_blank";
    link.rel = "noopener noreferrer";
    link.referrerPolicy = "no-referrer";
    link.textContent = "Data portraits";
    link.setAttribute(
      "aria-label",
      "Data portraits by Alexander Bogachev, opens external site",
    );
    const author = document.createElement("span");
    author.className = "portrait-credit-author";
    author.append(
      link,
      document.createTextNode(" by Alexander Bogachev."),
    );
    const socials = document.createElement("span");
    socials.className = "portrait-credit-socials";
    socials.append(
      socialCreditLink({
    ...
    disclosure.append(author, socials);
    syncDisclosure();
  }
  ...
  function openMatchDialog(record, card, { expanded = false } = {}) {
    ...
    originCard = card;
    renderModel(dialogContent, model, "mdlg", "h4");
    dialog.showModal();
