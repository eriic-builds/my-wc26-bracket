...
def _partial_record(code: str, calendar_match: Mapping[str, Any]) -> dict[str, Any]:
    ids = calendar_match.get("_ids")
    if not isinstance(ids, (list, tuple)) or len(ids) != 4 or not all(ids):
        raise MatchDetailError(f"{code} has no complete FIFA routing IDs")
    try:
        number = int(code[1:])
    ...
    record: dict[str, Any] = {
        ...
        "score": {
            "home": _score_value(calendar_match.get("gA"), f"{code}.gA"),
            "away": _score_value(calendar_match.get("gB"), f"{code}.gB"),
            "note": calendar_match.get("note") or "",
        },
        "winner": _required_text(calendar_match.get("winner"), f"{code}.winner"),
        "round": round_name,
...
def update_details(
    ...
    *,
    ...
    for code in sorted(result_map, key=lambda item: int(item[1:])):
        ...
        partial = _partial_record(code, calendar_match)
