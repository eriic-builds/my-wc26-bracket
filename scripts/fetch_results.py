#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Sync finished 2026 World Cup results into ``docs/data/results.json``.

The fan-viewer page is static and renders client-side, so this only writes the
shared results file (there is no per-entrant HTML to rebuild): fixtures and the
knockout topology are the same for every visitor.

What it does
------------
1. Reads the Round-of-32 fixtures and the knockout topology from
   ``docs/data/topology.json`` (entrant-independent — the same 16 pairings for all).
...
def _write_results(results: dict) -> None:
    """Persist docs/data/results.json. ensure_ascii=False keeps emoji/en-dashes
    readable; indent=1 matches the committed formatting so syncs stay minimal.
    The fan-viewer page is static and renders client-side, so there is nothing to
    rebuild — the browser fetches this file directly."""
...
def main() -> int:
    ...
    r32 = [(r[0], r[2], r[3]) for r in topo["r32"]]            # (code, teamA, teamB) — same for everyone
    ko_feed = {k: tuple(v) for k, v in topo["ko_feed"].items()}
    cur_res = {k: tuple(v) for k, v in results["res"].items()}
    cur_ko_fix = {k: tuple(v) for k, v in results.get("ko_fix", {}).items()}
    cur_auto_hl = [tuple(e) for e in results.get("auto_hl", [])]
    ...
    # Write docs/data/results.json deterministically (numeric code order, unicode
    # kept) so each sync produces a minimal diff. No HTML to rebuild — the page is
    # static and the browser fetches this file.
