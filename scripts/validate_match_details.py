...
def _validate_source(value: Any, path: str, errors: list[str]) -> None:
    ...
    if source.get("provider") != "FIFA":
        errors.append(f"{path}.provider: expected FIFA")
    for field in ("matchId",):
        if _text(source.get(field), f"{path}.{field}", errors):
...
def validate_portraits(
    ...
    if portraits.get("permission") != "required-for-production":
        errors.append("portraits.permission: expected required-for-production")
